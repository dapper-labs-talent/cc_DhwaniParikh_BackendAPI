...
// Customized version of the original jwtauth package from chi.
// Changed this to support our custom claims.
...
// Library errors
var (
	ErrUnauthorized = errors.New("auth: token is unauthorized")
	ErrExpired      = errors.New("auth: token is expired")
	ErrNBFInvalid   = errors.New("auth: token nbf validation failed")
	ErrIATInvalid   = errors.New("auth: token iat validation failed")
	ErrNoTokenFound = errors.New("auth: no token found")
	ErrAlgoInvalid  = errors.New("auth: algorithm mismatch")
)
...
// be the generic `auth.Authenticator` middleware or your own custom handler
...
func (ja *JWTAuth) Encode(claims JwtClaims) (t *jwt.Token, tokenString string, err error) {
	t = jwt.New(ja.signer)
	t.Claims = claims
...
func FromContext(ctx context.Context) (*jwt.Token, JwtClaims, error) {
	...
	if token != nil {
		if tokenClaims, ok := token.Claims.(*JwtClaims); ok {
			...
			panic(fmt.Sprintf("auth: unknown type of Claims: %T", token.Claims))
		}
	} else {
		claims = JwtClaims{}
	}
